...
$rc25_ano_vigente_delete = NULL; // Initialize page object first

class crc25_ano_vigente_delete extends crc25_ano_vigente {
	...
	// Page object name
	var $PageObjName = 'rc25_ano_vigente_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("rc25_ano_vigentelist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in rc25_ano_vigente class, rc25_ano_vigenteinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("rc25_ano_vigentelist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($rc25_ano_vigente_delete)) $rc25_ano_vigente_delete = new crc25_ano_vigente_delete();

// Page init
$rc25_ano_vigente_delete->Page_Init();

// Page main
$rc25_ano_vigente_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$rc25_ano_vigente_delete->Page_Render();
?>
...
var CurrentForm = frc25_ano_vigentedelete = new ew_Form("frc25_ano_vigentedelete", "delete");

// Form_CustomValidate event
frc25_ano_vigentedelete.Form_CustomValidate = 
...
frc25_ano_vigentedelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $rc25_ano_vigente_delete->ShowPageHeader(); ?>
<?php
$rc25_ano_vigente_delete->ShowMessage();
?>
<form name="frc25_ano_vigentedelete" id="frc25_ano_vigentedelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($rc25_ano_vigente_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $rc25_ano_vigente_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="rc25_ano_vigente">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($rc25_ano_vigente_delete->RecKeys as $key) { ?>
...
<?php if ($rc25_ano_vigente->ano_ano->Visible) { // ano_ano ?>
		<th class="<?php echo $rc25_ano_vigente->ano_ano->HeaderCellClass() ?>"><span id="elh_rc25_ano_vigente_ano_ano" class="rc25_ano_vigente_ano_ano"><?php echo $rc25_ano_vigente->ano_ano->FldCaption() ?></span></th>
<?php } ?>
<?php if ($rc25_ano_vigente->ano_valor_total->Visible) { // ano_valor_total ?>
		<th class="<?php echo $rc25_ano_vigente->ano_valor_total->HeaderCellClass() ?>"><span id="elh_rc25_ano_vigente_ano_valor_total" class="rc25_ano_vigente_ano_valor_total"><?php echo $rc25_ano_vigente->ano_valor_total->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$rc25_ano_vigente_delete->RecCnt = 0;
$i = 0;
while (!$rc25_ano_vigente_delete->Recordset->EOF) {
	$rc25_ano_vigente_delete->RecCnt++;
	$rc25_ano_vigente_delete->RowCnt++;

	// Set row properties
	$rc25_ano_vigente->ResetAttrs();
	$rc25_ano_vigente->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$rc25_ano_vigente_delete->LoadRowValues($rc25_ano_vigente_delete->Recordset);

	// Render row
	$rc25_ano_vigente_delete->RenderRow();
?>
	<tr<?php echo $rc25_ano_vigente->RowAttributes() ?>>
<?php if ($rc25_ano_vigente->ano_ano->Visible) { // ano_ano ?>
		<td<?php echo $rc25_ano_vigente->ano_ano->CellAttributes() ?>>
<span id="el<?php echo $rc25_ano_vigente_delete->RowCnt ?>_rc25_ano_vigente_ano_ano" class="rc25_ano_vigente_ano_ano">
...
<?php } ?>
<?php if ($rc25_ano_vigente->ano_valor_total->Visible) { // ano_valor_total ?>
		<td<?php echo $rc25_ano_vigente->ano_valor_total->CellAttributes() ?>>
<span id="el<?php echo $rc25_ano_vigente_delete->RowCnt ?>_rc25_ano_vigente_ano_valor_total" class="rc25_ano_vigente_ano_valor_total">
...
<?php
	$rc25_ano_vigente_delete->Recordset->MoveNext();
}
$rc25_ano_vigente_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $rc25_ano_vigente_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
frc25_ano_vigentedelete.Init();
</script>
<?php
$rc25_ano_vigente_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$rc25_ano_vigente_delete->Page_Terminate();
?>
