...
$rc25_a_termos_edit = NULL; // Initialize page object first

class crc25_a_termos_edit extends crc25_a_termos {
	...
	// Page object name
	var $PageObjName = 'rc25_a_termos_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			...
			// Load key from Form
			if ($objForm->HasValue("x_processo_id")) {
				$this->processo_id->setFormValue($objForm->GetValue("x_processo_id"));
			}
		} else {
	...
	function LoadFormValues() {
		...
		if (!$this->processo_id->FldIsDetailKey)
			$this->processo_id->setFormValue($objForm->GetValue("x_processo_id"));
	}
	...
	function RestoreFormValues() {
		global $objForm;
		$this->processo_id->CurrentValue = $this->processo_id->FormValue;
	...
	function ValidateForm() {
		...
		if (in_array("rc25_a_repasses", $DetailTblVar) && $GLOBALS["rc25_a_repasses"]->DetailEdit) {
		...
		if (in_array("rc25_a_planos_aplicacao", $DetailTblVar) && $GLOBALS["rc25_a_planos_aplicacao"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// processo_exercicio
			$this->processo_exercicio->SetDbValueDef($rsnew, $this->processo_exercicio->CurrentValue, NULL, $this->processo_exercicio->ReadOnly);

			// processo_termo_num
			$this->processo_termo_num->SetDbValueDef($rsnew, $this->processo_termo_num->CurrentValue, "", $this->processo_termo_num->ReadOnly);

			// processo_numero
			$this->processo_numero->SetDbValueDef($rsnew, $this->processo_numero->CurrentValue, "", $this->processo_numero->ReadOnly);

			// processo_vigencia_ini
			$this->processo_vigencia_ini->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->processo_vigencia_ini->CurrentValue, 6), NULL, $this->processo_vigencia_ini->ReadOnly);

			// processo_vigencia_fim
			$this->processo_vigencia_fim->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->processo_vigencia_fim->CurrentValue, 7), NULL, $this->processo_vigencia_fim->ReadOnly);

			// processo_data
			$this->processo_data->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->processo_data->CurrentValue, 7), NULL, $this->processo_data->ReadOnly);

			// processo_valor
			$this->processo_valor->SetDbValueDef($rsnew, $this->processo_valor->CurrentValue, NULL, $this->processo_valor->ReadOnly);

			// processo_objeto
			$this->processo_objeto->SetDbValueDef($rsnew, $this->processo_objeto->CurrentValue, NULL, $this->processo_objeto->ReadOnly);

			// processo_metas
			$this->processo_metas->SetDbValueDef($rsnew, $this->processo_metas->CurrentValue, NULL, $this->processo_metas->ReadOnly);

			// processo_origem
			$this->processo_origem->SetDbValueDef($rsnew, $this->processo_origem->CurrentValue, NULL, $this->processo_origem->ReadOnly);

			// processo_ent_endereco
			$this->processo_ent_endereco->SetDbValueDef($rsnew, $this->processo_ent_endereco->CurrentValue, NULL, $this->processo_ent_endereco->ReadOnly);

			// processo_ent_estatuto
			if ($this->processo_ent_estatuto->Visible && !$this->processo_ent_estatuto->ReadOnly && !$this->processo_ent_estatuto->Upload->KeepFile) {
			...
			// processo_ent_lei
			$this->processo_ent_lei->SetDbValueDef($rsnew, $this->processo_ent_lei->CurrentValue, NULL, $this->processo_ent_lei->ReadOnly);

			// processo_ent_cebas
			$this->processo_ent_cebas->SetDbValueDef($rsnew, $this->processo_ent_cebas->CurrentValue, NULL, $this->processo_ent_cebas->ReadOnly);

			// processo_resp_nome
			$this->processo_resp_nome->SetDbValueDef($rsnew, $this->processo_resp_nome->CurrentValue, NULL, $this->processo_resp_nome->ReadOnly);

			// processo_resp_cargo
			$this->processo_resp_cargo->SetDbValueDef($rsnew, $this->processo_resp_cargo->CurrentValue, NULL, $this->processo_resp_cargo->ReadOnly);

			// processo_resp_end
			$this->processo_resp_end->SetDbValueDef($rsnew, $this->processo_resp_end->CurrentValue, NULL, $this->processo_resp_end->ReadOnly);

			// processo_resp_contato
			$this->processo_resp_contato->SetDbValueDef($rsnew, $this->processo_resp_contato->CurrentValue, NULL, $this->processo_resp_contato->ReadOnly);

			// processo_resp_ata
			$this->processo_resp_ata->SetDbValueDef($rsnew, $this->processo_resp_ata->CurrentValue, NULL, $this->processo_resp_ata->ReadOnly);

			// processo_cont_nome
			$this->processo_cont_nome->SetDbValueDef($rsnew, $this->processo_cont_nome->CurrentValue, NULL, $this->processo_cont_nome->ReadOnly);

			// processo_cont_end
			$this->processo_cont_end->SetDbValueDef($rsnew, $this->processo_cont_end->CurrentValue, NULL, $this->processo_cont_end->ReadOnly);

			// processo_cont_contato
			$this->processo_cont_contato->SetDbValueDef($rsnew, $this->processo_cont_contato->CurrentValue, NULL, $this->processo_cont_contato->ReadOnly);

			// processo_cont_indent
			$this->processo_cont_indent->SetDbValueDef($rsnew, $this->processo_cont_indent->CurrentValue, NULL, $this->processo_cont_indent->ReadOnly);

			// processo_preenc_nome
			$this->processo_preenc_nome->SetDbValueDef($rsnew, $this->processo_preenc_nome->CurrentValue, NULL, $this->processo_preenc_nome->ReadOnly);

			// processo_preenc_carg
			$this->processo_preenc_carg->SetDbValueDef($rsnew, $this->processo_preenc_carg->CurrentValue, NULL, $this->processo_preenc_carg->ReadOnly);

			// processo_preenc_end
			$this->processo_preenc_end->SetDbValueDef($rsnew, $this->processo_preenc_end->CurrentValue, NULL, $this->processo_preenc_end->ReadOnly);

			// processo_preenc_contato
			$this->processo_preenc_contato->SetDbValueDef($rsnew, $this->processo_preenc_contato->CurrentValue, NULL, $this->processo_preenc_contato->ReadOnly);

			// processo_preenc_indentifica
			$this->processo_preenc_indentifica->SetDbValueDef($rsnew, $this->processo_preenc_indentifica->CurrentValue, NULL, $this->processo_preenc_indentifica->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("rc25_a_repasses", $DetailTblVar) && $GLOBALS["rc25_a_repasses"]->DetailEdit) {
						if (!isset($GLOBALS["rc25_a_repasses_grid"])) $GLOBALS["rc25_a_repasses_grid"] = new crc25_a_repasses_grid(); // Get detail page object
						$EditRow = $GLOBALS["rc25_a_repasses_grid"]->GridUpdate();
					}
				}
				if ($EditRow) {
					if (in_array("rc25_a_planos_aplicacao", $DetailTblVar) && $GLOBALS["rc25_a_planos_aplicacao"]->DetailEdit) {
						if (!isset($GLOBALS["rc25_a_planos_aplicacao_grid"])) $GLOBALS["rc25_a_planos_aplicacao_grid"] = new crc25_a_planos_aplicacao_grid(); // Get detail page object
						$EditRow = $GLOBALS["rc25_a_planos_aplicacao_grid"]->GridUpdate();
					}
				}
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("rc25_a_repasses", $DetailTblVar)) {
				...
				if ($GLOBALS["rc25_a_repasses_grid"]->DetailEdit) {
					$GLOBALS["rc25_a_repasses_grid"]->CurrentMode = "edit";
					$GLOBALS["rc25_a_repasses_grid"]->CurrentAction = "gridedit";
			...
			if (in_array("rc25_a_planos_aplicacao", $DetailTblVar)) {
				...
				if ($GLOBALS["rc25_a_planos_aplicacao_grid"]->DetailEdit) {
					$GLOBALS["rc25_a_planos_aplicacao_grid"]->CurrentMode = "edit";
					$GLOBALS["rc25_a_planos_aplicacao_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($rc25_a_termos_edit)) $rc25_a_termos_edit = new crc25_a_termos_edit();

// Page init
$rc25_a_termos_edit->Page_Init();

// Page main
$rc25_a_termos_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$rc25_a_termos_edit->Page_Render();
?>
...
var CurrentForm = frc25_a_termosedit = new ew_Form("frc25_a_termosedit", "edit");

// Validate form
frc25_a_termosedit.Validate = function() {
...
frc25_a_termosedit.Form_CustomValidate = 
...
frc25_a_termosedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Multi-Page
frc25_a_termosedit.MultiPage = new ew_MultiPage("frc25_a_termosedit");

// Dynamic selection lists
frc25_a_termosedit.Lists["x_processo_exercicio"] = {"LinkField":"x_ano_ano","Ajax":true,"AutoFill":false,"DisplayFields":["x_ano_ano","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"rc25_ano_vigente"};
frc25_a_termosedit.Lists["x_processo_exercicio"].Data = "<?php echo $rc25_a_termos_edit->processo_exercicio->LookupFilterQuery(FALSE, "edit") ?>";
...
<?php $rc25_a_termos_edit->ShowPageHeader(); ?>
<?php
$rc25_a_termos_edit->ShowMessage();
?>
<?php if (!$rc25_a_termos_edit->IsModal) { ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($rc25_a_termos_edit->Pager)) $rc25_a_termos_edit->Pager = new cPrevNextPager($rc25_a_termos_edit->StartRec, $rc25_a_termos_edit->DisplayRecs, $rc25_a_termos_edit->TotalRecs, $rc25_a_termos_edit->AutoHidePager) ?>
<?php if ($rc25_a_termos_edit->Pager->RecordCount > 0 && $rc25_a_termos_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($rc25_a_termos_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $rc25_a_termos_edit->PageUrl() ?>start=<?php echo $rc25_a_termos_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($rc25_a_termos_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $rc25_a_termos_edit->PageUrl() ?>start=<?php echo $rc25_a_termos_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $rc25_a_termos_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($rc25_a_termos_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $rc25_a_termos_edit->PageUrl() ?>start=<?php echo $rc25_a_termos_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($rc25_a_termos_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $rc25_a_termos_edit->PageUrl() ?>start=<?php echo $rc25_a_termos_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $rc25_a_termos_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="frc25_a_termosedit" id="frc25_a_termosedit" class="<?php echo $rc25_a_termos_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($rc25_a_termos_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $rc25_a_termos_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="rc25_a_termos">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($rc25_a_termos_edit->IsModal) ?>">
<div class="ewMultiPage"><!-- multi-page -->
<div class="nav-tabs-custom" id="rc25_a_termos_edit"><!-- multi-page .nav-tabs-custom -->
	<ul class="nav<?php echo $rc25_a_termos_edit->MultiPages->NavStyle() ?>">
		<li<?php echo $rc25_a_termos_edit->MultiPages->TabStyle("1") ?>><a href="#tab_rc25_a_termos1" data-toggle="tab"><?php echo $rc25_a_termos->PageCaption(1) ?></a></li>
		<li<?php echo $rc25_a_termos_edit->MultiPages->TabStyle("2") ?>><a href="#tab_rc25_a_termos2" data-toggle="tab"><?php echo $rc25_a_termos->PageCaption(2) ?></a></li>
		<li<?php echo $rc25_a_termos_edit->MultiPages->TabStyle("3") ?>><a href="#tab_rc25_a_termos3" data-toggle="tab"><?php echo $rc25_a_termos->PageCaption(3) ?></a></li>
		<li<?php echo $rc25_a_termos_edit->MultiPages->TabStyle("4") ?>><a href="#tab_rc25_a_termos4" data-toggle="tab"><?php echo $rc25_a_termos->PageCaption(4) ?></a></li>
		<li<?php echo $rc25_a_termos_edit->MultiPages->TabStyle("5") ?>><a href="#tab_rc25_a_termos5" data-toggle="tab"><?php echo $rc25_a_termos->PageCaption(5) ?></a></li>
	</ul>
	<div class="tab-content"><!-- multi-page .nav-tabs-custom .tab-content -->
		<div class="tab-pane<?php echo $rc25_a_termos_edit->MultiPages->PageStyle("1") ?>" id="tab_rc25_a_termos1"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($rc25_a_termos->processo_exercicio->Visible) { // processo_exercicio ?>
	<div id="r_processo_exercicio" class="form-group">
		<label id="elh_rc25_a_termos_processo_exercicio" for="x_processo_exercicio" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_exercicio->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_exercicio->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_termo_num->Visible) { // processo_termo_num ?>
	<div id="r_processo_termo_num" class="form-group">
		<label id="elh_rc25_a_termos_processo_termo_num" for="x_processo_termo_num" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_termo_num->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_termo_num->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_numero->Visible) { // processo_numero ?>
	<div id="r_processo_numero" class="form-group">
		<label id="elh_rc25_a_termos_processo_numero" for="x_processo_numero" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_numero->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_numero->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_vigencia_ini->Visible) { // processo_vigencia_ini ?>
	<div id="r_processo_vigencia_ini" class="form-group">
		<label id="elh_rc25_a_termos_processo_vigencia_ini" for="x_processo_vigencia_ini" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_vigencia_ini->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_vigencia_ini->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("frc25_a_termosedit", "x_processo_vigencia_ini", {"ignoreReadonly":true,"useCurrent":false,"format":6});
</script>
<?php } ?>
</span>
...
<?php if ($rc25_a_termos->processo_vigencia_fim->Visible) { // processo_vigencia_fim ?>
	<div id="r_processo_vigencia_fim" class="form-group">
		<label id="elh_rc25_a_termos_processo_vigencia_fim" for="x_processo_vigencia_fim" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_vigencia_fim->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_vigencia_fim->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("frc25_a_termosedit", "x_processo_vigencia_fim", {"ignoreReadonly":true,"useCurrent":false,"format":7});
</script>
<?php } ?>
</span>
...
<?php if ($rc25_a_termos->processo_data->Visible) { // processo_data ?>
	<div id="r_processo_data" class="form-group">
		<label id="elh_rc25_a_termos_processo_data" for="x_processo_data" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_data->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_data->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("frc25_a_termosedit", "x_processo_data", {"ignoreReadonly":true,"useCurrent":false,"format":7});
</script>
<?php } ?>
</span>
...
<?php if ($rc25_a_termos->processo_valor->Visible) { // processo_valor ?>
	<div id="r_processo_valor" class="form-group">
		<label id="elh_rc25_a_termos_processo_valor" for="x_processo_valor" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_valor->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_valor->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_objeto->Visible) { // processo_objeto ?>
	<div id="r_processo_objeto" class="form-group">
		<label id="elh_rc25_a_termos_processo_objeto" for="x_processo_objeto" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_objeto->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_objeto->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_metas->Visible) { // processo_metas ?>
	<div id="r_processo_metas" class="form-group">
		<label id="elh_rc25_a_termos_processo_metas" for="x_processo_metas" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_metas->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_metas->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_origem->Visible) { // processo_origem ?>
	<div id="r_processo_origem" class="form-group">
		<label id="elh_rc25_a_termos_processo_origem" for="x_processo_origem" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_origem->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_origem->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $rc25_a_termos_edit->MultiPages->PageStyle("2") ?>" id="tab_rc25_a_termos2"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($rc25_a_termos->processo_ent_endereco->Visible) { // processo_ent_endereco ?>
	<div id="r_processo_ent_endereco" class="form-group">
		<label id="elh_rc25_a_termos_processo_ent_endereco" for="x_processo_ent_endereco" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_ent_endereco->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_ent_endereco->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_ent_estatuto->Visible) { // processo_ent_estatuto ?>
	<div id="r_processo_ent_estatuto" class="form-group">
		<label id="elh_rc25_a_termos_processo_ent_estatuto" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_ent_estatuto->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_ent_estatuto->CellAttributes() ?>>
...
<?php if (@$_POST["fa_x_processo_ent_estatuto"] == "0") { ?>
<input type="hidden" name="fa_x_processo_ent_estatuto" id= "fa_x_processo_ent_estatuto" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_processo_ent_estatuto" id= "fa_x_processo_ent_estatuto" value="1">
<?php } ?>
...
<?php if ($rc25_a_termos->processo_ent_lei->Visible) { // processo_ent_lei ?>
	<div id="r_processo_ent_lei" class="form-group">
		<label id="elh_rc25_a_termos_processo_ent_lei" for="x_processo_ent_lei" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_ent_lei->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_ent_lei->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_ent_cebas->Visible) { // processo_ent_cebas ?>
	<div id="r_processo_ent_cebas" class="form-group">
		<label id="elh_rc25_a_termos_processo_ent_cebas" for="x_processo_ent_cebas" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_ent_cebas->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_ent_cebas->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $rc25_a_termos_edit->MultiPages->PageStyle("3") ?>" id="tab_rc25_a_termos3"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($rc25_a_termos->processo_resp_nome->Visible) { // processo_resp_nome ?>
	<div id="r_processo_resp_nome" class="form-group">
		<label id="elh_rc25_a_termos_processo_resp_nome" for="x_processo_resp_nome" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_resp_nome->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_resp_nome->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_resp_cargo->Visible) { // processo_resp_cargo ?>
	<div id="r_processo_resp_cargo" class="form-group">
		<label id="elh_rc25_a_termos_processo_resp_cargo" for="x_processo_resp_cargo" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_resp_cargo->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_resp_cargo->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_resp_end->Visible) { // processo_resp_end ?>
	<div id="r_processo_resp_end" class="form-group">
		<label id="elh_rc25_a_termos_processo_resp_end" for="x_processo_resp_end" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_resp_end->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_resp_end->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_resp_contato->Visible) { // processo_resp_contato ?>
	<div id="r_processo_resp_contato" class="form-group">
		<label id="elh_rc25_a_termos_processo_resp_contato" for="x_processo_resp_contato" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_resp_contato->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_resp_contato->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_resp_ata->Visible) { // processo_resp_ata ?>
	<div id="r_processo_resp_ata" class="form-group">
		<label id="elh_rc25_a_termos_processo_resp_ata" for="x_processo_resp_ata" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_resp_ata->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_resp_ata->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $rc25_a_termos_edit->MultiPages->PageStyle("4") ?>" id="tab_rc25_a_termos4"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($rc25_a_termos->processo_cont_nome->Visible) { // processo_cont_nome ?>
	<div id="r_processo_cont_nome" class="form-group">
		<label id="elh_rc25_a_termos_processo_cont_nome" for="x_processo_cont_nome" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_cont_nome->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_cont_nome->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_cont_end->Visible) { // processo_cont_end ?>
	<div id="r_processo_cont_end" class="form-group">
		<label id="elh_rc25_a_termos_processo_cont_end" for="x_processo_cont_end" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_cont_end->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_cont_end->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_cont_contato->Visible) { // processo_cont_contato ?>
	<div id="r_processo_cont_contato" class="form-group">
		<label id="elh_rc25_a_termos_processo_cont_contato" for="x_processo_cont_contato" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_cont_contato->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_cont_contato->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_cont_indent->Visible) { // processo_cont_indent ?>
	<div id="r_processo_cont_indent" class="form-group">
		<label id="elh_rc25_a_termos_processo_cont_indent" for="x_processo_cont_indent" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_cont_indent->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_cont_indent->CellAttributes() ?>>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $rc25_a_termos_edit->MultiPages->PageStyle("5") ?>" id="tab_rc25_a_termos5"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
<?php if ($rc25_a_termos->processo_preenc_nome->Visible) { // processo_preenc_nome ?>
	<div id="r_processo_preenc_nome" class="form-group">
		<label id="elh_rc25_a_termos_processo_preenc_nome" for="x_processo_preenc_nome" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_preenc_nome->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_preenc_nome->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_preenc_carg->Visible) { // processo_preenc_carg ?>
	<div id="r_processo_preenc_carg" class="form-group">
		<label id="elh_rc25_a_termos_processo_preenc_carg" for="x_processo_preenc_carg" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_preenc_carg->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_preenc_carg->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_preenc_end->Visible) { // processo_preenc_end ?>
	<div id="r_processo_preenc_end" class="form-group">
		<label id="elh_rc25_a_termos_processo_preenc_end" for="x_processo_preenc_end" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_preenc_end->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_preenc_end->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_preenc_contato->Visible) { // processo_preenc_contato ?>
	<div id="r_processo_preenc_contato" class="form-group">
		<label id="elh_rc25_a_termos_processo_preenc_contato" for="x_processo_preenc_contato" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_preenc_contato->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_preenc_contato->CellAttributes() ?>>
...
<?php if ($rc25_a_termos->processo_preenc_indentifica->Visible) { // processo_preenc_indentifica ?>
	<div id="r_processo_preenc_indentifica" class="form-group">
		<label id="elh_rc25_a_termos_processo_preenc_indentifica" for="x_processo_preenc_indentifica" class="<?php echo $rc25_a_termos_edit->LeftColumnClass ?>"><?php echo $rc25_a_termos->processo_preenc_indentifica->FldCaption() ?></label>
		<div class="<?php echo $rc25_a_termos_edit->RightColumnClass ?>"><div<?php echo $rc25_a_termos->processo_preenc_indentifica->CellAttributes() ?>>
...
<input type="hidden" data-table="rc25_a_termos" data-field="x_processo_id" name="x_processo_id" id="x_processo_id" value="<?php echo ew_HtmlEncode($rc25_a_termos->processo_id->CurrentValue) ?>">
<?php
	if (in_array("rc25_a_repasses", explode(",", $rc25_a_termos->getCurrentDetailTable())) && $rc25_a_repasses->DetailEdit) {
?>
...
<?php } ?>
<?php
	if (in_array("rc25_a_planos_aplicacao", explode(",", $rc25_a_termos->getCurrentDetailTable())) && $rc25_a_planos_aplicacao->DetailEdit) {
?>
<?php if ($rc25_a_termos->getCurrentDetailTable() <> "") { ?>
...
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $rc25_a_termos_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $rc25_a_termos_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
<script type="text/javascript">
frc25_a_termosedit.Init();
</script>
<?php
$rc25_a_termos_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$rc25_a_termos_edit->Page_Terminate();
?>
