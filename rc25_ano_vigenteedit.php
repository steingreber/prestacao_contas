...
$rc25_ano_vigente_edit = NULL; // Initialize page object first

class crc25_ano_vigente_edit extends crc25_ano_vigente {
	...
	// Page object name
	var $PageObjName = 'rc25_ano_vigente_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			...
			// Load key from Form
			if ($objForm->HasValue("x_ano_id")) {
				$this->ano_id->setFormValue($objForm->GetValue("x_ano_id"));
			}
		} else {
	...
	function RestoreFormValues() {
		global $objForm;
		$this->ano_id->CurrentValue = $this->ano_id->FormValue;
	...
	function EditRow() {
		...
		if ($this->ano_ano->CurrentValue <> "") { // Check field with unique index
			$sFilterChk = "(`ano_ano` = " . ew_AdjustSql($this->ano_ano->CurrentValue, $this->DBID) . ")";
		...
		if ($rs->EOF) {
			...
			// ano_ano
			$this->ano_ano->SetDbValueDef($rsnew, $this->ano_ano->CurrentValue, 0, $this->ano_ano->ReadOnly);

			// ano_descri
			$this->ano_descri->SetDbValueDef($rsnew, $this->ano_descri->CurrentValue, "", $this->ano_descri->ReadOnly);

			// ano_valor_total
			$this->ano_valor_total->SetDbValueDef($rsnew, $this->ano_valor_total->CurrentValue, 0, $this->ano_valor_total->ReadOnly);

			// ano_vigencia_ini
			$this->ano_vigencia_ini->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->ano_vigencia_ini->CurrentValue, 7), ew_CurrentDate(), $this->ano_vigencia_ini->ReadOnly);

			// ano_vigencia_fim
			$this->ano_vigencia_fim->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->ano_vigencia_fim->CurrentValue, 7), ew_CurrentDate(), $this->ano_vigencia_fim->ReadOnly);

			// ano_prest_contas
			$this->ano_prest_contas->SetDbValueDef($rsnew, $this->ano_prest_contas->CurrentValue, "", $this->ano_prest_contas->ReadOnly);
...
// Create page object
if (!isset($rc25_ano_vigente_edit)) $rc25_ano_vigente_edit = new crc25_ano_vigente_edit();

// Page init
$rc25_ano_vigente_edit->Page_Init();

// Page main
$rc25_ano_vigente_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$rc25_ano_vigente_edit->Page_Render();
?>
...
var CurrentForm = frc25_ano_vigenteedit = new ew_Form("frc25_ano_vigenteedit", "edit");

// Validate form
frc25_ano_vigenteedit.Validate = function() {
...
frc25_ano_vigenteedit.Form_CustomValidate = 
...
frc25_ano_vigenteedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $rc25_ano_vigente_edit->ShowPageHeader(); ?>
<?php
$rc25_ano_vigente_edit->ShowMessage();
?>
<?php if (!$rc25_ano_vigente_edit->IsModal) { ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($rc25_ano_vigente_edit->Pager)) $rc25_ano_vigente_edit->Pager = new cPrevNextPager($rc25_ano_vigente_edit->StartRec, $rc25_ano_vigente_edit->DisplayRecs, $rc25_ano_vigente_edit->TotalRecs, $rc25_ano_vigente_edit->AutoHidePager) ?>
<?php if ($rc25_ano_vigente_edit->Pager->RecordCount > 0 && $rc25_ano_vigente_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($rc25_ano_vigente_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $rc25_ano_vigente_edit->PageUrl() ?>start=<?php echo $rc25_ano_vigente_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($rc25_ano_vigente_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $rc25_ano_vigente_edit->PageUrl() ?>start=<?php echo $rc25_ano_vigente_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $rc25_ano_vigente_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($rc25_ano_vigente_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $rc25_ano_vigente_edit->PageUrl() ?>start=<?php echo $rc25_ano_vigente_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($rc25_ano_vigente_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $rc25_ano_vigente_edit->PageUrl() ?>start=<?php echo $rc25_ano_vigente_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $rc25_ano_vigente_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="frc25_ano_vigenteedit" id="frc25_ano_vigenteedit" class="<?php echo $rc25_ano_vigente_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($rc25_ano_vigente_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $rc25_ano_vigente_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="rc25_ano_vigente">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($rc25_ano_vigente_edit->IsModal) ?>">
<div class="ewEditDiv"><!-- page* -->
<?php if ($rc25_ano_vigente->ano_ano->Visible) { // ano_ano ?>
	<div id="r_ano_ano" class="form-group">
		<label id="elh_rc25_ano_vigente_ano_ano" for="x_ano_ano" class="<?php echo $rc25_ano_vigente_edit->LeftColumnClass ?>"><?php echo $rc25_ano_vigente->ano_ano->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $rc25_ano_vigente_edit->RightColumnClass ?>"><div<?php echo $rc25_ano_vigente->ano_ano->CellAttributes() ?>>
...
<?php if ($rc25_ano_vigente->ano_descri->Visible) { // ano_descri ?>
	<div id="r_ano_descri" class="form-group">
		<label id="elh_rc25_ano_vigente_ano_descri" for="x_ano_descri" class="<?php echo $rc25_ano_vigente_edit->LeftColumnClass ?>"><?php echo $rc25_ano_vigente->ano_descri->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $rc25_ano_vigente_edit->RightColumnClass ?>"><div<?php echo $rc25_ano_vigente->ano_descri->CellAttributes() ?>>
...
<?php if ($rc25_ano_vigente->ano_valor_total->Visible) { // ano_valor_total ?>
	<div id="r_ano_valor_total" class="form-group">
		<label id="elh_rc25_ano_vigente_ano_valor_total" for="x_ano_valor_total" class="<?php echo $rc25_ano_vigente_edit->LeftColumnClass ?>"><?php echo $rc25_ano_vigente->ano_valor_total->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $rc25_ano_vigente_edit->RightColumnClass ?>"><div<?php echo $rc25_ano_vigente->ano_valor_total->CellAttributes() ?>>
...
<?php if ($rc25_ano_vigente->ano_vigencia_ini->Visible) { // ano_vigencia_ini ?>
	<div id="r_ano_vigencia_ini" class="form-group">
		<label id="elh_rc25_ano_vigente_ano_vigencia_ini" for="x_ano_vigencia_ini" class="<?php echo $rc25_ano_vigente_edit->LeftColumnClass ?>"><?php echo $rc25_ano_vigente->ano_vigencia_ini->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $rc25_ano_vigente_edit->RightColumnClass ?>"><div<?php echo $rc25_ano_vigente->ano_vigencia_ini->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("frc25_ano_vigenteedit", "x_ano_vigencia_ini", {"ignoreReadonly":true,"useCurrent":false,"format":7});
</script>
<?php } ?>
</span>
...
<?php if ($rc25_ano_vigente->ano_vigencia_fim->Visible) { // ano_vigencia_fim ?>
	<div id="r_ano_vigencia_fim" class="form-group">
		<label id="elh_rc25_ano_vigente_ano_vigencia_fim" for="x_ano_vigencia_fim" class="<?php echo $rc25_ano_vigente_edit->LeftColumnClass ?>"><?php echo $rc25_ano_vigente->ano_vigencia_fim->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $rc25_ano_vigente_edit->RightColumnClass ?>"><div<?php echo $rc25_ano_vigente->ano_vigencia_fim->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("frc25_ano_vigenteedit", "x_ano_vigencia_fim", {"ignoreReadonly":true,"useCurrent":false,"format":7});
</script>
<?php } ?>
</span>
...
<?php if ($rc25_ano_vigente->ano_prest_contas->Visible) { // ano_prest_contas ?>
	<div id="r_ano_prest_contas" class="form-group">
		<label id="elh_rc25_ano_vigente_ano_prest_contas" for="x_ano_prest_contas" class="<?php echo $rc25_ano_vigente_edit->LeftColumnClass ?>"><?php echo $rc25_ano_vigente->ano_prest_contas->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $rc25_ano_vigente_edit->RightColumnClass ?>"><div<?php echo $rc25_ano_vigente->ano_prest_contas->CellAttributes() ?>>
...
<input type="hidden" data-table="rc25_ano_vigente" data-field="x_ano_id" name="x_ano_id" id="x_ano_id" value="<?php echo ew_HtmlEncode($rc25_ano_vigente->ano_id->CurrentValue) ?>">
<?php if (!$rc25_ano_vigente_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $rc25_ano_vigente_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $rc25_ano_vigente_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
<script type="text/javascript">
frc25_ano_vigenteedit.Init();
</script>
<?php
$rc25_ano_vigente_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$rc25_ano_vigente_edit->Page_Terminate();
?>
